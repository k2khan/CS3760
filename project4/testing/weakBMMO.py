...
#Best on the leaderboard so far
#Move ordering,bestmove, weak heuristic
class submission3(Player):
  ...
  def moveOrdering(self, state):
    ...
    if state in self._bestMove:
      locations[self._bestMove.get(state)] = locations.get(self._bestMove.get(state), 0) + 100000
